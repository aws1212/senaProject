...
<?php
    ...
    {
            $sqlvd = "SELECT imagen FROM code_representante_Mixta";
            ...
            if($rowvd == null)
            {
                if(move_uploaded_file($_FILES["imagee"]["tmp_name"],"image_repreMixta/".$_FILES["imagee"]["name"]))
                {
                    $linkimg_Diurna = "http://buzonctt.iswin.be/buzonctt/php/image_repreMixta/".$_FILES["imagee"]["name"];

                    $sqlivs = "INSERT INTO code_representante_Mixta(code,imagen)VALUES('$code','$linkimg_Diurna')";
                
                    $queryivd=sqlsrv_query($conn,$sqlivs);
        
                    if($queryivd){

                        $sqlsvd = "SELECT code FROM code_representante_Mixta";
            ...
            else
            {
                ...
                else
                {
                    if(move_uploaded_file($_FILES["imagee"]["tmp_name"],"image_repreMixta/".$_FILES["imagee"]["name"]))
                    {
                        ...
                        $sqllos = "UPDATE code_representante_Mixta set code = '$code', imagen = '$linkimg_Diurna'";
                
                        $queryuvd=sqlsrv_query($conn,$sqllos);
            
                        if($queryuvd){

                            $sqllov = "SELECT code FROM code_representante_Mixta";
